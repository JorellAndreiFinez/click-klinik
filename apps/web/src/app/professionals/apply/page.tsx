...
import {
  checkDoctorMobileAvailability,
  checkDoctorSignupEligibility,
  submitDoctorApplication,
} from "@/lib/doctor-api";
import { getFirebaseAuth, isFirebaseConfigured } from "@/lib/firebase";

type Notice = { kind: "error" | "success"; message: string } | null;
type DoctorSignUpStep =
  | "account"
  | "password"
  | "mobile"
  | "profile"
...
export default function DoctorApplicationPage() {
  ...
  async function handleEmailSignUp(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice({
        kind: "success",
        message:
          "Your professional account has been created. Add a contact number to continue.",
      });
    } catch (error) {
  ...
  async function handleGoogleSignUp() {
    ...
    try {
      ...
      if (!credential.user.email) {
        throw new Error(
          "Your Google account does not include an email address.",
        );
      }
      await checkDoctorSignupEligibility(credential.user);
  ...
  async function handleGooglePasswordSetup(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice({
        kind: "success",
        message: "Your account is secured. Add a contact number to continue.",
      });
    } catch (error) {
      setNotice({ kind: "error", message: getAuthErrorMessage(error) });
    } finally {
      ...
      setNotice({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Unable to check your contact number.",
      });
    } finally {
      setBusy(false);
    }
  ...
  async function handleSubmitProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await submitDoctorApplication(user, {
        ...
        otherSpecialization:
          String(form.get("otherSpecialization") ?? "").trim() || undefined,
        clinicOrHospital:
          String(form.get("clinicOrHospital") ?? "").trim() || undefined,
        location: String(form.get("location") ?? "").trim() || undefined,
        regionCode: String(form.get("regionCode") ?? "").trim(),
        regionName: String(form.get("regionName") ?? "").trim(),
        provinceCode:
          String(form.get("provinceCode") ?? "").trim() || undefined,
        provinceName:
          String(form.get("provinceName") ?? "").trim() || undefined,
            ...
            <section className="order-first px-5 py-7 sm:px-10 lg:order-last lg:px-11 lg:py-10">
              <div className="mx-auto max-w-xl">
                ...
                <h1 className="mt-3 text-3xl font-bold tracking-tight">
                  Apply as a telehealth doctor.
                </h1>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                  Create a protected account first, then submit credentials for
                  review before doctor access is activated.
                </p>
                ...
                {step === "submitted" && (
                  <SubmittedApplication firstName={firstName} />
                )}
              </div>
            </section>
          </div>
    ...
    <aside className="relative hidden overflow-hidden bg-primary p-9 text-primary-foreground lg:block">
      ...
      <div className="relative">
        ...
        <h2 className="mt-5 text-3xl leading-tight font-bold">
          Serbisyong medikal, mas malapit sa pamilya.
        </h2>
        <p className="mt-4 text-sm leading-7 text-primary-foreground/68">
          Your professional identity is reviewed before patients can discover or
          book your services.
        </p>
        <div className="mt-10 space-y-3">
          <GuideItem
            done={step !== "account" && step !== "password"}
            icon={<Stethoscope />}
            title="Secure professional account"
          />
          <GuideItem
            done={step === "profile" || step === "submitted"}
            icon={<Phone />}
            title="Registered contact number"
          />
          <GuideItem
            done={step === "submitted"}
            icon={<FileCheck2 />}
            title="Credentials submitted for review"
          />
          <GuideItem
            done={false}
            icon={<CalendarDays />}
            title="Scheduling after approval"
          />
        </div>
        <p className="mt-10 text-xs leading-6 text-primary-foreground/55">
          Click Klinik handles professional details only for access review and
          telehealth operations under its privacy notice.
        </p>
      </div>
    </aside>
  );
...
function GuideItem({
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl bg-primary-foreground/[0.07] p-4">
      <span
        className={`flex size-9 items-center justify-center rounded-lg ${done ? "bg-secondary text-primary" : "bg-primary-foreground/10 text-secondary"}`}
      >
...
function DoctorProgress({ step }: { step: DoctorSignUpStep }) {
  ...
  const currentStep =
    step === "account" || step === "password" ? 1 : step === "mobile" ? 2 : 3;
  ...
  return (
    ...
    >
      <span
        className="absolute top-3 right-[17%] left-[17%] h-px bg-border"
        aria-hidden="true"
      />
      {labels.map((label, index) => (
        ...
        >
          <span
            className={`flex size-6 items-center justify-center rounded-full text-[11px] font-bold ${index + 1 <= currentStep ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
            {index + 1}
          </span>
          <span
            className={`text-xs font-semibold ${index + 1 <= currentStep ? "text-primary" : "text-muted-foreground"}`}
          >
            {label}
          </span>
        </div>
  ...
  return (
    <div className="mt-7">
      <Button
        ...
        className="h-12 w-full rounded-xl border-primary/15 bg-background"
      >
        <span className="flex size-7 items-center justify-center rounded-full bg-card text-base font-bold text-[#4285F4] shadow-sm">
          G
        </span>
        Sign up with Google
      </Button>
    ...
    <form className="mt-7 grid gap-4" onSubmit={onSubmit}>
      <div className="rounded-xl bg-primary/5 p-4">
        ...
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          Google sign-up succeeded. Create a backup password before continuing
          your application.
        </p>
    ...
    <div className="mt-7">
      <div className="flex items-start gap-3 rounded-xl bg-secondary/24 p-4">
        ...
        <p className="text-xs leading-5 text-muted-foreground">
          For the MVP, we check that this Philippine mobile number is not
          already used in another doctor application. No SMS OTP is sent.
        </p>
      </div>
      <form className="mt-5 grid gap-4" onSubmit={onSubmit}>
        <Field required label="Professional mobile number">
          <div className="flex overflow-hidden rounded-xl border border-input bg-background focus-within:ring-3 focus-within:ring-ring/45">
            <span className="flex items-center border-r border-border px-4 text-sm font-bold text-primary">
              +63
            </span>
            <input
              required
              value={mobileNumber}
              onChange={(event) =>
                setMobileNumber(toPhilippineLocalDigits(event.target.value))
              }
  ...
  return (
    <form className="mt-7 grid gap-4 sm:grid-cols-2" onSubmit={onSubmit}>
      <div className="rounded-xl bg-primary/5 p-4 sm:col-span-2">
        ...
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          Account email: {accountEmail} | Contact: {mobileNumber}
        </p>
      </div>
      <ApplicationField
        required
      ...
      {suffix === "Other" && (
        <ApplicationField
          ...
          name="otherSuffix"
          label="Other suffix"
          placeholder="Professional credential"
        />
      )}
      <ApplicationField
        required
      ...
      <div className="min-w-0 sm:col-span-2">
        <ApplicationField
          ...
          label="Clinic or hospital affiliation"
      ...
      <div className="rounded-xl bg-primary/5 p-4 sm:col-span-2">
        ...
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          Choose your region, city or municipality, and barangay using PSGC
          location data.
        </p>
      </div>
      ...
      <label className="grid gap-2 sm:col-span-2">
        ...
        <textarea
          ...
          placeholder="Share your teleconsult experience, patient groups, and care focus..."
        />
      </label>
      <label className="flex items-start justify-between gap-4 rounded-2xl border border-primary/10 bg-background p-4 sm:col-span-2">
        <span>
          <span className="block text-sm font-bold">
            Display my profile publicly after approval
          </span>
          <span className="mt-1 block text-xs leading-5 text-muted-foreground">
            When enabled, this doctor can appear on public-facing pages only
            after credential approval.
          </span>
        </span>
        <input
          type="checkbox"
      ...
      <label className="flex items-start gap-3 rounded-2xl border border-border p-4 text-xs leading-5 text-muted-foreground sm:col-span-2">
        <input
          required
          name="credentialReviewConsent"
          type="checkbox"
          className="mt-1 accent-primary"
        />
        <span>
          I confirm these details are accurate and consent to credential review
          and secure processing for telehealth onboarding.
          <RequiredMark />
        </span>
      </label>
      <Button
        disabled={busy}
        className="mt-2 h-12 w-full rounded-xl sm:col-span-2"
      >
...
function SubmittedApplication({ firstName }: { firstName: string }) {
  return (
    <div className="mt-7 rounded-2xl bg-primary px-6 py-9 text-center text-primary-foreground">
      ...
      <p className="mt-4 text-xs font-bold tracking-[0.18em] text-secondary uppercase">
      ...
      <p className="mt-3 text-sm leading-6 text-primary-foreground/70">
        Your account has been successfully created. For this hackathon
        demonstration, doctor accounts are automatically approved, allowing
        immediate access to scheduling, consultations, and patient management
        features.
      </p>
      <Button
        asChild
        className="mt-7 h-12 rounded-xl bg-secondary px-7 text-secondary-foreground hover:bg-secondary/90"
      >
  ...
  return (
    <Field required={required} label={label}>
      <select
        ...
        className="h-12 w-full min-w-0 rounded-xl border border-input bg-background px-3 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50"
      >
        {children}
      </select>
    </Field>
  ...
  return (
    <div className="relative min-w-0">
      ...
      <button
        ...
        className="absolute top-0 right-0 flex h-12 w-12 items-center justify-center text-muted-foreground hover:text-primary"
      >
...
function AuthNotice({ notice }: { notice: Exclude<Notice, null> }) {
  return (
    <div
      role="alert"
      className={`mt-6 rounded-xl border px-4 py-3 text-sm ${notice.kind === "error" ? "border-destructive/20 bg-destructive/6 text-destructive" : "border-[#12734b]/20 bg-[#e6f7ee] text-[#12734b]"}`}
    >
      {notice.message}
    </div>
  );
...
function FirebaseSetupNotice() {
  return (
    <div
      role="alert"
      className="mt-6 rounded-xl border border-secondary bg-secondary/16 px-4 py-3 text-xs leading-6 text-foreground"
    >
      <p className="font-bold">Authentication setup required</p>
      <p className="mt-1 text-muted-foreground">
        Add Firebase Web App values in{" "}
        <code className="font-mono">apps/web/.env.local</code>, then restart the
        web server.
      </p>
    </div>
  );
  ...
  if (error instanceof FirebaseError) {
    const knownErrors: Record<string, string> = {
      ...
      "auth/provider-already-linked":
        "This login method is already attached to your account.",
      "auth/weak-password": passwordPolicy,
      "auth/popup-closed-by-user":
        "Google sign-up was closed before completion.",
      "auth/too-many-requests": "Too many attempts. Please try again later.",
    };
    return (
      knownErrors[error.code] ?? "Unable to create your professional account."
    );
  }
  return error instanceof Error
    ? error.message
    : "Unable to complete professional onboarding.";
}
